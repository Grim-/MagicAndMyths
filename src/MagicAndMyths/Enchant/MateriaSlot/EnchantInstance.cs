...
namespace MagicAndMyths
{
    public class EnchantInstance : IExposable
    {
        ...
        public virtual DamageWorker.DamageResult Notify_ApplyMeleeDamageToTarget(LocalTargetInfo target, Pawn Attacker, ref DamageWorker.DamageResult DamageWorkerResult)
        {
            ...
            Log.Message($"EnchantInstance Notify_ApplyMeleeDamageToTarget, active effects {activeEffects.Count}");

            foreach (var effect in activeEffects)
            {
                damageResult = effect.Notify_ApplyMeleeDamageToTarget(target, Attacker, ref DamageWorkerResult);
            }
