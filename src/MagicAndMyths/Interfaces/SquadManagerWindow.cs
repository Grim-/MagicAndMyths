...
namespace MagicAndMyths
{
    public class SquadManagerWindow : Window
    {
        ...
        {
            // Get the currently selected pawn that might be a squad leader
            ...
            if (selectedPawn != null && selectedPawn.Faction == Faction.OfPlayer)
            {
                ...
                // If the selected pawn isn't a leader, try to find a squad leader
        ...
        {
            // Check if we have a leader selected
            if (currentLeader == null)
            {
        ...
        private void DrawToolbar(Rect rect)
        {
            ...
            // Add new squad button (right-aligned)
        ...
        private void DrawSquadsList(Rect rect)
        {
            ...
            foreach (var squadEntry in currentLeader.ActiveSquads)
            {
                ...
                if (Widgets.ButtonText(deleteRect, "Remove"))
                {
                    ...
                    break; // Break out of the loop as we've modified the collection
                }
        ...
        private void DrawMemberRow(Pawn pawn, float width, float yPos, bool isLeader = false)
        {
            ...
            // Remove member button (not for leader)
            if (!isLeader)
            {
                // Take remaining space minus button width
                ...
                {
                    // Find which squad this pawn belongs to
        ...
        private float CalculateTotalHeight()
        {
            ...
            foreach (var squad in currentLeader.ActiveSquads.Values)
            {
                height += rowHeight; // Squad header

                // Count members (including leader)
                int memberCount = 0;
                if (squad.Leader != null) memberCount++;
                if (squad.Members != null) memberCount += squad.Members.Count;
                if (squad.Leader != null && squad.Members.Contains(squad.Leader)) memberCount--; // Don't double count leader

                height += memberCount * memberRowHeight;
                height += 10f; // Spacing between squads
            }

            return Math.Max(height, 50f); // Ensure minimum height
        }
    }
}
