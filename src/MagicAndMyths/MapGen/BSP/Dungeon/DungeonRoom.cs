...
namespace MagicAndMyths
{
    public class DungeonRoom
    {
        ...
        public int DifficultyTier
        {
            ...
            {
                int maxTiers = 5;
                return Mathf.Clamp(Mathf.FloorToInt(ProgressionValue * maxTiers), 0, maxTiers - 1);
            }
        }
        ...
        public void AddConnectionTo(Map map, DungeonRoom OtherRoom, List<Corridoor> corridoors = null)
        {
            if (!HasConnectionTo(OtherRoom))
            {
                ...
                if (corridoors != null)
                {
                    newConnection.corridors = corridoors.ToList();
                }
                else
                {
        ...
        public IEnumerable<IntVec3> EntranceCells()
        {
            ...
            {
                foreach (var corridoors in item.corridors)
                {
                    yield return corridoors.Start;
                    yield return corridoors.End;
                }
            }
        }
