...
namespace MagicAndMyths
{
    public class WorldCustomSiteManager : WorldComponent
    {
        ...
        public Map GetOrCreateMapForTile(int tileId, List<SitePartDef> sitePartDefs = null)
        {
            ...
            Map newMap = CreateNewMap(tileId, sitePartDefs);
            return newMap;
        }


        private Map CreateNewMap(int tileId, List<SitePartDef> sitePartDefs = null)
        {
            ...
            if (newSite != null)
            {
                ...
                Map newMap = MapGenerator.GenerateMap(Find.World.info.initialMapSize, mapParent, MagicAndMythDefOf.Portal_EmptyMap, null, null, true);
                ...
                //dungeonManager.AddGenerator(new DungeonGen_BaseGen(newMap));
                //dungeonManager.AddGenerator(new DungeonGen_CaveSystem(newMap));
                //dungeonManager.AddGenerator(new DungeonGen_TerrainAndWalls(newMap));
        ...
        public Map CreateCustomDimension(int tileId, Building_PortalGate portal, Map sourceMap, IntVec3 mapSize, List<SitePartDef> sitePartDefs = null)
        {
            var mapParent = (CustomMapParent)WorldObjectMaker.MakeWorldObject(MagicAndMythDefOf.CustomMapParent);
            ...
            Map pocketMap = MapGenerator.GenerateMap(
                mapSize,
                mapParent,
                MagicAndMythDefOf.Portal_EmptyMap,
                null,
                null,
                true
            );
            ...
            DungeonGen dungeonGen = new DungeonGen_GenerateEnemies(pocketMap);
            dungeonGen.Generate();
            Find.World.info.seedString = originalSeed;

            storedSites.Add(new StoredSiteData(tileId, null, mapParent));
            // StoredDimensionMaps[GateAddress.ToInt()] = mapParent;
            return pocketMap;
        }

        public Site GetSiteForTileID(int tileId)
        {
            return storedSites.FirstOrDefault(s => s.tileId == tileId)?.site;
        }
        ...
        public override void ExposeData()
        {
            ...
            {
                // Only remove entries where the site itself is null
                storedSites.RemoveAll(data => data.site == null);
